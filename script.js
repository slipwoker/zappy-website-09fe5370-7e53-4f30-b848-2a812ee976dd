document.addEventListener('DOMContentLoaded', function() {
  ...
  if (calculatorForm) {
    calculatorForm.addEventListener('submit', function(e) {
      ...
      const activities = [
        { id: 'tenantComm', hours: 'tenantCommHours' },
        { id: 'maintenance', hours: 'maintenanceHours' },
        { id: 'emergencies', hours: 'emergenciesHours' },
        { id: 'rentCollection', hours: 'rentCollectionHours' },
        { id: 'inspections', hours: 'inspectionsHours' },
        { id: 'marketing', hours: 'marketingHours' },
        { id: 'screening', hours: 'screeningHours' },
        { id: 'accounting', hours: 'accountingHours' },
        { id: 'legal', hours: 'legalHours' }
      ];
      ...
      activities.forEach(activity => {
        ...
        if (checkbox && checkbox.checked && hoursInput) {
          totalHours += parseFloat(hoursInput.value) || 0;
        }
      });
      
      const hourlyRate = parseFloat(document.getElementById('hourlyRate').value) || 50;
      const monthlyRent = parseFloat(document.getElementById('monthlyRent').value) || 2000;
      ...
      const pmcCost = monthlyRent * numUnits * 0.09;
      const savings = opportunityCost - pmcCost;
      
      document.getElementById('totalHours').textContent = totalHours.toFixed(1);
      document.getElementById('opportunityCost').textContent = '$' + opportunityCost.toFixed(0);
      document.getElementById('annualCost').textContent = '$' + annualCost.toFixed(0);
      document.getElementById('selfManagementCost').textContent = '$' + opportunityCost.toFixed(0) + '/חודש';
      document.getElementById('pmcCost').textContent = '$' + pmcCost.toFixed(0) + '/חודש';
      document.getElementById('selfTimeInvestment').textContent = totalHours.toFixed(1);
      
      let savingsText = '';
      let recommendationText = '';
      
      if (savings > 0) {
        savingsText = 'על ידי מעבר לחברת ניהול מקצועית, תוכל לחסוך <strong>$' + savings.toFixed(0) + ' בחודש</strong> ($' + (savings * 12).toFixed(0) + ' בשנה). זה לא כולל את הערך של הזמן והשקט הנפשי שתקבל.';
        recommendationText = '<p><strong>המלצה:</strong> על סמך הנתונים שלך, נראה שמעבר לחברת ניהול מקצועית יהיה משתלם עבורך. אתה משקיע ' + totalHours.toFixed(1) + ' שעות בחודש, ששוות $' + opportunityCost.toFixed(0) + ', בעוד שחברת ניהול תעלה לך רק $' + pmcCost.toFixed(0) + ' בחודש.</p><p>בנוסף, חברת ניהול מקצועית תספק לך שקט נפשי, ניהול מקצועי יותר, וזמינות 24/7 לשוכרים. אתה תוכל להשקיע את הזמן שלך בדברים חשובים יותר - כמו רכישת נכסים נוספים או פיתוח העסק שלך.</p>';
      } else {
        savingsText = 'עלות הניהול העצמי שלך נמוכה יותר מחברת ניהול ב-<strong>$' + Math.abs(savings).toFixed(0) + ' בחודש</strong>. עם זאת, חשוב לקחת בחשבון גם את השקט הנפשי והזמינות שחברת ניהול מספקת.';
        recommendationText = '<p><strong>המלצה:</strong> מבחינה כלכלית גרידא, הניהול העצמי שלך משתלם. אתה משקיע ' + totalHours.toFixed(1) + ' שעות בחודש, ששוות $' + opportunityCost.toFixed(0) + ', בעוד שחברת ניהול תעלה $' + pmcCost.toFixed(0) + ' בחודש.</p><p>עם זאת, שקול גם את הגורמים הבאים: האם אתה נהנה מהניהול? האם יש לך זמן פנוי? האם הניהול מפריע לעבודה או למשפחה שלך? לפעמים שקט הנפש והזמינות שחברת ניהול מספקת שווים את ההוצאה הנוספת.</p>';
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מחשבון פורטפוליו משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            const response = await fetch('https://api.zappy5.com/api/email/contact-form', {
                ...
                body: JSON.stringify({
                    websiteId: '09fe5370-7e53-4f30-b848-2a812ee976dd',
                    name: data.name || '',
                    email: data.email || '',
